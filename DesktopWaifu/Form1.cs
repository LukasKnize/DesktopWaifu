...
namespace DesktopWaifu
{
    public partial class DesktopWaifu : Form
    {
        ...
        private void getWaifu2(bool nsfw, string tag)
        {
            //TextField.Text += TextField.Text.Length == 0 ? "" : "Loading";
            Loading.Visible = true;
            this.Refresh();
            Cursor.Current = Cursors.WaitCursor;
            string img_url = Getter.getWaifu2(nsfw, tag);
            Common.ImgHistory.Add(img_url);
            Display.Load(Common.ImgHistory.Current.path);
            //TextField.Text += TextField.Text.Length == 0 ? "" : ", Done\n";
            Loading.Visible = false;
        ...
        private void executeCommand(string command) {
            ...
            string[] sfwTags2 = { "uniform", "maid", "marin-kitagawa", "mori-calliope", "raiden-shogun", "oppai", "selfies"};
            string[] nsfwTags2 = { "ass", "hentai", "milf", "oral", "paizuri", "ecchi", "ero" };
            ...
            if (command == "?") {
                ...
                if (splitedCommand[1] == "sfw") {
                    if (sfwTags.Contains(splitedCommand[2]))
                    {
                        getWaifu(splitedCommand[1] + "/" + splitedCommand[2]);
                    }
                    else if(sfwTags2.Contains(splitedCommand[2]))
                    {
                        getWaifu2(false, splitedCommand[2]);
                    }
                    else
                    {
                ...
                else if (splitedCommand[1] == "nsfw") {
                    if (nsfwTags.Contains(splitedCommand[2]))
                    {
                        getWaifu(splitedCommand[1] + "/" + splitedCommand[2]);
                    }
                    else if (nsfwTags2.Contains(splitedCommand[2]))
                    {
                        getWaifu2(true, splitedCommand[2]);
                    }
                    else
                    {
                        Output.Text += "invalid command, try ?" + "\n";
